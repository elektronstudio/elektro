<script setup lang="ts">
const tilesHorisontal = 20;
</script>

<template>
  <div class="EBreadBoard"></div>
</template>

<style scoped>
.EBreadBoard {
  --tile-size: calc(100vw / v-bind(tilesHorisontal));

  position: fixed;
  top: 0;
  left: 0;
  width: 100%;
  height: 100%;
  background-image: url(data:image/svg+xml;base64,);
  background-repeat: repeat;
  background-size: var(--tile-size);
}
</style>
